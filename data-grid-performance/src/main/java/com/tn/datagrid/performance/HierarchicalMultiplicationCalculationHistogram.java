...
public class HierarchicalMultiplicationCalculationHistogram extends CalculationHistogram
{
  ...
  private static final IntValue C = new IntValue(new NumericIdentity<>(INT_TYPE, 3), 3);
  private static final Identity<Integer, IntValue> CALCULATED_IDENTITY_2 = new CalculatedIdentity<>(INT_TYPE, Operators.multiply(), B.getIdentity(), C.getIdentity());
  private static final Identity<Integer, IntValue> CALCULATED_IDENTITY_1 = new CalculatedIdentity<>(INT_TYPE, Operators.multiply(), A.getIdentity(), CALCULATED_IDENTITY_2);
  ...
  protected void test()
  {
    ...
    IntValue result = calculatedIntegers.get(CALCULATED_IDENTITY_1);
    recordValue(System.nanoTime() - start);

    if (result.get() != A.get() * (B.get() * C.get()))
    {
